...
describe('ensureMemoryFile', () => {
  ...
  it('creates the config home and file for the user memory target', () => {
    const path = join(home, '.alego-tui', 'ALEGO-TUI.md')
  ...
  it('preserves existing content (exclusive create swallows EEXIST)', () => {
    const path = join(home, 'ALEGO-TUI.md')
  ...
  it('propagates non-EEXIST errors (missing dir outside the config home)', () => {
    expect(() => ensureMemoryFile(join(home, 'nope', 'ALEGO-TUI.md'), home)).toThrow()
  })
})
...
describe('relativeMemoryPath', () => {
  it('prefers the shorter of ~ and ./ spellings', () => {
    expect(relativeMemoryPath('/home/u/w/ALEGO-TUI.md', '/home/u/w', '/home/u')).toBe('./ALEGO-TUI.md')
    expect(relativeMemoryPath('/home/u/.alego-tui/ALEGO-TUI.md', '/home/u/deep/nested/dir', '/home/u')).toBe(
      '~/.alego-tui/ALEGO-TUI.md'
    )
  })

  it('falls back to the absolute path outside home and cwd', () => {
    expect(relativeMemoryPath('/etc/ALEGO-TUI.md', '/home/u/w', '/home/u')).toBe('/etc/ALEGO-TUI.md')
  })
})
...
describe('openMemoryFileInEditor', () => {
  ...
  it('creates the file, spawns inside the suspend, notifies, and reports', async () => {
    const d = deps()
    const path = join(cwd, 'ALEGO-TUI.md')
    ...
    expect(d.calls).toEqual([
      ...
      'suspend:exit',
      'notify',
      `sys:Opened memory file at ./ALEGO-TUI.md\n\n${editorHint({ EDITOR: 'vim' })}`
    ])
  })
  ...
  it('reports a launch failure and skips the edited notification', async () => {
    ...
    await openMemoryFileInEditor(join(cwd, 'ALEGO-TUI.md'), d)
  ...
  it('reports ensure-create failures without spawning', async () => {
    ...
    await openMemoryFileInEditor(join(cwd, 'missing-dir', 'ALEGO-TUI.md'), d)
