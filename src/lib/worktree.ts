 ...
 * and advertises it via ALEGO_TUI_WORKTREE_* env vars (the same block the
 ...
 *
 * Ownership gate: the launcher stamps ALEGO_TUI_WORKTREE_OWNER_PID with its
...
export function parseWorktreeSession(
  ...
  const name = env.ALEGO_TUI_WORKTREE_NAME
  const path = env.ALEGO_TUI_WORKTREE_PATH
  const branch = env.ALEGO_TUI_WORKTREE_BRANCH
  const originalCwd = env.ALEGO_TUI_WORKTREE_ORIGINAL_CWD
  const repoRoot = env.ALEGO_TUI_WORKTREE_REPO_ROOT

  if (!name || !path || !branch || !originalCwd || !repoRoot) {
    return null
  }

  const ownerPid = Number(env.ALEGO_TUI_WORKTREE_OWNER_PID ?? '')
