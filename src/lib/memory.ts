...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because ALEGO_TUI_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.ALEGO_TUI_HEAPDUMP_DIR?.trim() || appHomePath('heapdumps')
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.ALEGO_TUI_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.ALEGO_TUI_HEAPDUMP_MAX_BYTES?.trim()
