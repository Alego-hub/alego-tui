import type { MouseTrackingMode } from '@alego-tui/ink'
...
export const STARTUP_RESUME_ID = (process.env.ALEGO_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.ALEGO_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.ALEGO_TUI_IMAGE ?? '').trim()
...
//
// - ALEGO_TUI_MOUSE_TRACKING (truthy/falsy) explicitly overrides everything.
//   This is the "force a value" knob and intentionally beats the legacy
//   kill-switch and the Termux default.
// - ALEGO_TUI_DISABLE_MOUSE=1 forces mouse off — the legacy kill switch.
...
//   behavior.
const mouseTrackingOverride = parseToggle(process.env.ALEGO_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.ALEGO_TUI_DISABLE_MOUSE)
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.ALEGO_TUI_NO_CONFIRM)
...
export const DASHBOARD_TUI_MODE = truthy(process.env.ALEGO_TUI_DASHBOARD)

// ALEGO_TUI_DEV_CREDITS — dev-only live-spend readout (Δ status segment + "(dev credits)"
// banner). Throwaway dev scaffolding; the whole readout gates on this one flag.
export const DEV_CREDITS_MODE = truthy(process.env.ALEGO_TUI_DEV_CREDITS)

const inlineOverride = parseToggle(process.env.ALEGO_TUI_INLINE)
...
// Opt into the fullscreen/alternate-screen experience with ALEGO_TUI_INLINE=0.
export const INLINE_MODE = inlineOverride ?? true

// Live FPS counter overlay, fed by ink's onFrame (real render rate, not a
// synthetic timer).
export const SHOW_FPS = truthy(process.env.ALEGO_TUI_FPS)
