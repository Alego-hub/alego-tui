 ...
 *
 * Set ALEGO_TUI_RENDER_DEBUG=1 to record both. Output goes to a FILE, never
 ...
 * override with ALEGO_TUI_RENDER_DEBUG_FILE.
 */
...
const enabled = (): boolean => {
  const v = process.env.ALEGO_TUI_RENDER_DEBUG

  return !!v && v !== '0' && v !== 'false'
}

const target = (): string => process.env.ALEGO_TUI_RENDER_DEBUG_FILE || join(tmpdir(), 'clawcodex-render-debug.log')
