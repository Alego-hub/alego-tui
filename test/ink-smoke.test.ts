// Stage 2 acceptance: the vendored @alego-tui/ink fork builds and renders headlessly.
...
describe('vendored @alego-tui/ink fork', () => {
  it('renders Box/Text synchronously into a fake TTY', async () => {
    const { Box, Text, renderSync } = await import('@alego-tui/ink')
    ...
    const instance = renderSync(
      createElement(
        ...
        createElement(Text, { color: 'green' }, 'alego-tui smoke')
      ),
    ...
    expect(plain).toContain('alego-tui')
